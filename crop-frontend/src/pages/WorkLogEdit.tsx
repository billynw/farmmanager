...
import { workTypesApi, workLogsApi } from '../api'
import type { AgroInput } from '../api'
import AppHeader from '../components/AppHeader'
import BottomNav from '../components/BottomNav'

interface AgroRow { id?: number; product_name: string; quantity: string; dilution: string; unit: string }
...
export default function WorkLogEdit() {
  ...
  const [agroRows, setAgroRows] = useState<AgroRow[]>([])
  const [deletePhotoIds, setDeletePhotoIds] = useState<number[]>([])

  const { data: log } = useQuery({ queryKey: ['work-log', logIdNum], queryFn: () => workLogsApi.get(logIdNum).then(r => r.data) })
  const { data: workTypes = [] } = useQuery({ queryKey: ['work-types'], queryFn: () => workTypesApi.list().then(r => r.data) })

  useEffect(() => {
    if (log) {
      setWorkTypeId(log.work_type?.id)
      setWorkedAt(new Date(log.worked_at).toISOString().slice(0, 16))
      setMemo(log.memo ?? '')
      if (log.agro_inputs.length > 0) {
        setShowAgro(true)
        setAgroRows(log.agro_inputs.map((a: AgroInput) => ({
          id: a.id,
        ...
        setAgroRows([{ product_name: '', quantity: '', dilution: '', unit: 'L' }])
      }
    }
  }, [log])
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      await workLogsApi.update(logIdNum, {
        item_id: itemId,
        ...
        agro_inputs: showAgro ? agroRows.filter(r => r.product_name).map(r => ({
          ...
          dilution: r.dilution || undefined,
          unit: r.unit || undefined,
        })) : [],
      })
      for (const photoId of deletePhotoIds) {
        await workLogsApi.deletePhoto(logIdNum, photoId)
      }
    },
    onSuccess: () => {
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
              {log.photos.map(p => (
                <div key={p.id} style={{ position: 'relative' }}>
                  <img src={p.file_path} alt="" style={{
                    width: 80, height: 80, objectFit: 'cover', borderRadius: 8,
                    opacity: deletePhotoIds.includes(p.id) ? 0.3 : 1,
                  }} />
                  <button onClick={() => toggleDeletePhoto(p.id)} style={{
                    position: 'absolute', top: 2, right: 2,
                    width: 20, height: 20, borderRadius: '50%',
                    background: deletePhotoIds.includes(p.id) ? '#2d7a4f' : '#e74c3c',
                    color: '#fff', border: 'none', cursor: 'pointer', fontSize: 12,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
                    {deletePhotoIds.includes(p.id) ? '↩' : '×'}
                  </button>
                </div>
              ))}
            </div>
